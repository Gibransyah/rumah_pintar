...
export default function UploadPengajarPage() {
  ...
  const [isError, setIsError] = useState(false); // State untuk indikator error
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!file) {
      ...
      setIsError(true);
      setIsLoading(false); // Nonaktifkan loading jika tidak ada file
      return;
    }
    ...
    try {
      const res = await fetch("/api/admin/pengajar/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (res.ok) {
        setMessage(data.message || "Unggah berhasil!");
        setIsError(false);
        setFile(null); // Reset file input
        toast.success(data.message || "Unggah berhasil!"); // Tampilkan toast sukses
      } else {
        setMessage(data.message || "Unggah gagal. Terjadi kesalahan.");
        setIsError(true);
        toast.error(data.message || "Unggah gagal."); // Tampilkan toast error
      }
    } catch (error: any) {
      console.error("Error saat mengunggah file:", error);
      setMessage(error.message || "Terjadi kesalahan jaringan atau server.");
      setIsError(true);
      toast.error(error.message || "Unggah gagal."); // Tampilkan toast error umum
    } finally {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="mb-6 text-3xl font-bold text-gray-800">Unggah Data Pengajar</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-xl shadow-lg max-w-md mx-auto">
        <p className="mb-4 text-gray-600">
          Unggah file Excel (.xlsx atau .xls) yang berisi daftar pengajar baru.
          Pastikan format kolom sesuai: **Nama, Email, Password, dan Mata Pelajaran**.
        </p>
        ...
        <button
          type="submit"
          className="w-full rounded-lg bg-green-600 py-3.5 font-semibold text-white shadow-md transition duration-300 ease-in-out hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 disabled:cursor-not-allowed disabled:opacity-60"
