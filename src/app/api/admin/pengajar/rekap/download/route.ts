...
export async function GET() {
  try {
    ...
    // Menggunakan nullish coalescing operator (??) untuk 'bidang' agar string kosong jika null/undefined.
    const data = pengajar.map((p) => ({
      ID: p.id,
      Nama: p.nama,
      Email: p.user.email, // Ambil email dari relasi user
      Bidang: (p as any).bidang ?? '', // Menggunakan 'as any' jika 'bidang' tidak ada di tipe Prisma Pengajar
    }));
  ...
  } catch (err) { // Menggunakan 'err' sesuai kode asli Anda
    console.error('Gagal membuat rekap pengajar:', err);
